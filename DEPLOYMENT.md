...
第一次部署时，不要急着一次改很多东西。建议按这个顺序：

1. 先确认 Python 和虚拟环境正常
2. 再确认 FastAPI 能启动
3. 再确认页面能打开
4. 再配置 API Key
5. 最后再测试聊天和会话容器
